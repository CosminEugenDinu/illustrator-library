...
var jsonString = JSON.stringify(papers,null,' ');

 
function saveStringAsJson (string){
            var file = new File(Folder.desktop + "/papers.json");
            //~ file.open("e", "TEXT", "????");
            file.open("e"); // creates or open file on user desktop
...
// this should be run later...
function importJsonAsObj () {
    var f = new File(Folder.desktop+"/papers.json");
    f.open('e'); // open for edit (read and write);
    var s = f.read(); // string;
    f.close();
    var obFromJson = JSON.parse(s);
    return obFromJson;
}


// this should be run later... wait for the first function to finish

//~ var obj = importJsonAsObj();
